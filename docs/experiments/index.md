...
このドキュメントでは、Single-view、Fixed Ring-4 MVCNN、Learned Circular-4 MVTNを比較するための共通設計を説明します。
...
本実験へ進む前に、10クラスだけのdebug subsetで3条件を確認します。
...
debug subsetが通ったら、全採用アセットで3条件を実行します。
