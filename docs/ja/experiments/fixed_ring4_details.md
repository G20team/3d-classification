...
各splitではyaw/elevation offsetが加わります。標準splitは17角度をtrain、validation、testへ層化しているため、
角度分布を揃えた状態で固定4視点のカバー範囲とCNN特徴を評価します。同一のポケモンID・角度の組はsplit間で重複しません。
...
全条件で同じmanifest、同じmesh cache、同じ `pose_splits_stratified_seed0.json` を使うことで、条件間の差分を視点数と視点配置に集中させます。
