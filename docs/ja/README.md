...
ポケモンを角度分布を揃えたtrain/validation/testから識別するclosed-set分類です。
研究上の主な問いは、限られた4視点という条件で、学習カメラ配置やattentionによるview統合が、
固定円環状カメラとmax poolingによる識別より有効か、です。
...
| View Transformer-4 | 共有ResNet18の4視点特徴をTransformerで統合する条件 | `configs/view_transformer4.yaml` |
...
- [実験設計の概要](experiments/index.md): 4条件の比較設計、共通設定、debug subset、本実験の進め方。
...
- [View Transformer-4](experiments/view_transformer4.md): attentionによる4視点統合条件。
